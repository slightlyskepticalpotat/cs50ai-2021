        ...
        for i in range(self.height):
            for j in range(self.width):
                if (i, j) == cell or (i, j) in self.safes:
                    continue
                elif max(abs(i - cell[0]), abs(j - cell[1])) <= 1:
                    peers.add((i, j))
        self.knowledge.append(Sentence(peers, count))

        # 4 and 5
        # cant change while iterating
        safes = set()
        mines = set()
        for sentence in self.knowledge:
            for square in sentence.known_safes():
                safes.add(square)
            for square in sentence.known_mines():
                mines.add(square)
        for square in safes:
            ...
            for sentence2 in self.knowledge:
                if sentence1.cells != sentence2.cells:
                    if sentence1.cells.issubset(sentence2.cells):
                        ...
                        to_add.append(
                            Sentence(sentence1.cells - sentence2.cells, sentence1.count - sentence2.count))
